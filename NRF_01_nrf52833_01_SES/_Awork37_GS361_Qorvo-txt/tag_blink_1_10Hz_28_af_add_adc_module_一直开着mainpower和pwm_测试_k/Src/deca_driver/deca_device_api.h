...
#define BIT_MASK(bit_num)   (((uint32)1)<<(bit_num))
...
#define dwt_set_bit_num_8bit_reg(addr,bit_num) dwt_modify8bitoffsetreg(addr,0,-1,BIT_MASK(bit_num))
#define dwt_clr_bit_num_8bit_reg(addr,bit_num) dwt_modify8bitoffsetreg(addr,0,~BIT_MASK(bit_num),0)
...
#define dwt_set_bit_num_16bit_reg(addr,bit_num) dwt_modify16bitoffsetreg(addr,0,-1,BIT_MASK(bit_num))
#define dwt_clr_bit_num_16bit_reg(addr,bit_num) dwt_modify16bitoffsetreg(addr,0,~BIT_MASK(bit_num),0)
...
#define dwt_set_bit_num_32bit_reg(addr,bit_num) dwt_modify32bitoffsetreg(addr,0,-1,BIT_MASK(bit_num))
#define dwt_clr_bit_num_32bit_reg(addr,bit_num) dwt_modify32bitoffsetreg(addr,0,~BIT_MASK(bit_num),0)
...
#define GPIO_PIN2_RXLED         (((uint32)0x1)<<6)    /* The pin operates as the RXLED output */
#define GPIO_PIN3_TXLED         (((uint32)0x1)<<9)    /* The pin operates as the TXLED output */

#define GPIO_PIN0_EXTTXE        (((uint32)0x2)<<0)    /* The pin operates as the EXTTXE output (output TX state) */
#define GPIO_PIN1_EXTRXE        (((uint32)0x2)<<3)    /* The pin operates as the EXTRXE output (output RX state) */

#define GPIO_PIN4_EXTDA         (((uint32)0x1)<<12)   /* The pin operates to support external DA/PA */
#define GPIO_PIN5_EXTTX         (((uint32)0x1)<<15)   /* The pin operates to support external PA */
#define GPIO_PIN6_EXTRX         (((uint32)0x1)<<18)   /* The pin operates to support external LNA */
...
#define DWT_CB_DATA_RX_FLAG_ND  0x2 // No data mode
#define DWT_CB_DATA_RX_FLAG_CIA 0x4 // CIA done
#define DWT_CB_DATA_RX_FLAG_CER 0x8 // CIA error
...
#define GET_STS_REG_SET_VALUE(x)     ((uint16)1<<((x)+2))    /* Returns the value to set in CP_CFG0_ID for STS length. The x is the enum value from dwt_sts_lengths_e */
...
typedef struct
{
    uint8 chan ;           //!< Channel number (5 or 9)
    uint8 txPreambLength ; //!< DWT_PLEN_64..DWT_PLEN_4096
    uint8 rxPAC ;          //!< Acquisition Chunk Size (Relates to RX preamble length)
    uint8 txCode ;         //!< TX preamble code (the code configures the PRF, e.g. 9 -> PRF of 64 MHz)
    uint8 rxCode ;         //!< RX preamble code (the code configures the PRF, e.g. 9 -> PRF of 64 MHz)
    uint8 sfdType;         //!< SFD type (0 for short IEEE 8-bit standard, 1 for DW 8-bit, 2 for DW 16-bit, 3 for 4z BPRF)
    uint8 dataRate ;       //!< Data rate {DWT_BR_850K or DWT_BR_6M8}
    uint8 phrMode ;        //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 - extended frames DWT_PHRMODE_EXT}
    uint8 phrRate;         //!< PHR rate {0x0 - standard DWT_PHRRATE_STD, 0x1 - at datarate DWT_PHRRATE_DTA}
    uint16 sfdTO ;         //!< SFD timeout value (in symbols)
    uint8 stsMode;      //!< STS mode (no STS, STS before PHR or STS after data)
    dwt_sts_lengths_e stsLength;    //!< STS length (the allowed values are listed in dwt_sts_lengths_e
    uint8 pdoaMode;        //!< PDOA mode
} dwt_config_t ;
...
typedef struct
{
    uint8       ipatovRxTime[5] ;   // RX timestamp from Ipatov sequence
    uint8       ipatovRxStatus ;    // RX status info for Ipatov sequence
    uint16      ipatovPOA ;         // POA of Ipatov

    uint8       stsRxTime[5] ;   // RX timestamp from STS
    uint16      stsRxStatus ;    // RX status info for STS
    uint16      stsPOA;          // POA of STS block 1
    uint8       sts2RxTime[5];   // RX timestamp from STS
    uint16      sts2RxStatus;    // RX status info for STS
    uint16      sts2POA;         // POA of STS block 2

    uint8       tdoa[6];            // TDOA from two STS RX timestamps
    int16       pdoa;               // PDOA from two STS POAs signed int [1:-11] in radians

    int16       xtalOffset ;        // estimated xtal offset of remote device
    uint32      ciaDiag1 ;          // Diagnostics common to both sequences

    uint32      ipatovPeak ;        // index and amplitude of peak sample in Ipatov sequence CIR
    uint32      ipatovPower ;       // channel area allows estimation of channel power for the Ipatov sequence
    uint32      ipatovF1 ;          // F1 for Ipatov sequence
    uint32      ipatovF2 ;          // F2 for Ipatov sequence
    uint32      ipatovF3 ;          // F3 for Ipatov sequence
    uint16      ipatovFpIndex ;     // First path index for Ipatov sequence 
    uint16      ipatovAccumCount ;  // Number accumulated symbols for Ipatov sequence

    uint32      stsPeak ;        // index and amplitude of peak sample in STS CIR
    uint16      stsPower ;       // channel area allows estimation of channel power for the STS
    uint32      stsF1 ;          // F1 for STS
    uint32      stsF2 ;          // F2 for STS
    uint32      stsF3 ;          // F3 for STS
    uint16      stsFpIndex ;     // First path index for STS
    uint16      stsAccumCount ;  // Number accumulated symbols for STS

    uint32      sts2Peak;        // index and amplitude of peak sample in STS CIR
    uint16      sts2Power;       // channel area allows estimation of channel power for the STS
    uint32      sts2F1;          // F1 for STS
    uint32      sts2F2;          // F2 for STS
    uint32      sts2F3;          // F3 for STS
    uint16      sts2FpIndex;     // First path index for STS
    uint16      sts2AccumCount;  // Number accumulated symbols for STS
...
typedef struct
{
    ...
    uint16 PHE;                    //12-bit number of received header error events
    uint16 RSL;                    //12-bit number of received frame sync loss event events
    uint16 CRCG;                   //12-bit number of good CRC received frame events
    uint16 CRCB;                   //12-bit number of bad CRC (CRC error) received frame events
    uint8  ARFE;                   //8-bit number of address filter error events
    uint8  OVER;                   //8-bit number of receive buffer overrun events (used in double buffer mode)
    uint16 SFDTO;                  //12-bit number of SFD timeout events
    uint16 PTO;                    //12-bit number of Preamble timeout events
    uint8  RTO;                    //8-bit number of RX frame wait timeout events
    uint16 TXF;                    //12-bit number of transmitted frame events
    uint8  HPW;                    //8-bit half period warning events (when delayed RX/TX enable is used)
    uint8  CRCE;                   //8-bit SPI CRC error events
    uint16 PREJ;                   //12-bit number of Preamble rejection events
...
typedef enum
{
    AES_key_otp_type_RAM=0,/*Core type GCM*/
    AES_key_otp_type_OTP/*Core type CCM*/
...
typedef struct {
    ...
    dwt_aes_key_src_e       key_src;        //!< Location of the key: either as programmed in registers(128 bit) or in the RAM
...
typedef enum
{
	CH5_MAX_GAIN_NF = 0x1c0fd,
	CH5_MAX_GAIN_MINUS_12DB_NF_PLUS_6DB = 0x1c43e,
	CH5_MAX_GAIN_MINUS_17DB_NF_PLUS_10DB = 0x1c6be,
	CH5_MAX_GAIN_MINUS_28DB_NF_PLUS_16DB = 0x1c77e,
	CH5_MAX_GAIN_MINUS_32DB_NF_PLUS_23DB = 0x1cf36,
	CH5_MAX_GAIN_MINUS_37DB_NF_PLUS_29DB = 0x1cfb5,
	CH5_MAX_GAIN_MINUS_39DB_NF_PLUS_30DB = 0x1cff5
...
typedef enum
{
	CH9_MAX_GAIN_NF = 0x2a8fe,
	CH9_MAX_GAIN_MINUS_12DB_NF_PLUS_4DB = 0x2ac36,
	CH9_MAX_GAIN_MINUS_20DB_NF_PLUS_10DB = 0x2a5fe,
	CH9_MAX_GAIN_MINUS_24DB_NF_PLUS_16DB = 0x2af3e,
	CH9_MAX_GAIN_MINUS_30DB_NF_PLUS_20DB = 0x2af7d,
	CH9_MAX_GAIN_MINUS_33DB_NF_PLUS_24DB = 0x2afb5,
	CH9_MAX_GAIN_MINUS_40DB_NF_PLUS_28DB = 0x2afb5
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function initiates communications with the DW3000 transceiver
 * and reads its DEV_ID register (address 0x00) to verify the IC is one supported
 * by this software (e.g. DW3000 32-bit device ID value is 0xDECA0200).  Then it
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function configures the STS AES 128 bit key value, the default value is [31:0]c9a375fa, [63:32]8df43a20, [95:64]b5e5a4ed, [127:96]0738123b
 *        Any combination of 128 bits can be used.
 *
 * input parameters
 ...
 *        A value of all 0 is invalid and DW3000 will automatically detect this condition and set it to 1.
 *
 * input parameters
 ...
 * no return value
 */
void dwt_configurestsiv(uint8* stsIvBalueBytes);
 ...
 * no return value
 */
void dwt_signal_rx_buff_free(/*dwt_rx_buff_type_e   dwt_rx_buff_type*/);
 ...
 *
 * input parameters
 * @param aon_address - this is the addres of the memory location to read
 *
 * output parameters - None
 *
 ...
 *
 * @param aon_address - this is the addres of the memory location to write
 ...
 */
uint8 dwt_generatecrc8(const uint8* byteArray, int flen, uint8 crcInit);
 ...
 *
 * input parameters
 * @param stsstatus - the (uint8) STS status value.
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @brief This function enables repeated frames to be generated given a frame repitition rate.
 *
 * input parameters:
...
(
    uint32      recordNumber,   // input parameter - ID of register file or buffer being accessed
    uint16      index,          // input parameter - byte index into register file or buffer being accessed
    uint16      length,         // input parameter - number of bytes being written
...
(
    uint32  recordNumber,       // input parameter - ID of register file or buffer being accessed
    uint16  index,              // input parameter - byte index into register file or buffer being accessed
    uint16  length,             // input parameter - number of bytes being read
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
extern int writetospiwithcrc(uint16 headerLength, const uint8 *headerBuffer, uint16 bodylength, const uint8 *bodyBuffer, uint8 crc8);
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
extern int writetospi(uint16 headerLength, const uint8 *headerBuffer, uint16 bodylength, const uint8 *bodyBuffer);
 ...
 */
extern int readfromspi(uint16 headerLength, /*const*/ uint8 *headerBuffer, uint16 readlength, uint8 *readBuffer);
...
* no return value
*/
extern uint16 spi_cs_low_delay(uint16 delay_ms);
